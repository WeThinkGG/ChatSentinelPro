...
public class VelocityModuleManager extends ModuleManager {
	...
	public void reloadData() {
        ...
        getCapsModule().loadData(configYml.node("caps", "enabled").getBoolean(), configYml.node("caps", "replace").getBoolean(),
		...
		getCooldownModule().loadData(configYml.node("cooldown", "enabled").getBoolean(),
				configYml.node("cooldown", "time", "repeat-global").getInt(), configYml.node("cooldown", "time", "repeat").getInt(),
				configYml.node("cooldown", "time", "normal").getInt(), configYml.node("cooldown", "time", "command").getInt());
		getFloodModule().loadData(configYml.node("flood", "enabled").getBoolean(), configYml.node("flood", "replace").getBoolean(),
		...
		getBlacklistModule().loadData(configYml.node("blacklist", "enabled").getBoolean(),
				configYml.node("blacklist", "fake_message").getBoolean(), configYml.node("blacklist", "hide_words").getBoolean(),
				configYml.node("blacklist", "warn", "max").getInt(), configYml.node("blacklist", "warn", "notification").getString(),
				...
				blacklistYml.node("expressions").childrenList().stream()
						.map(ConfigurationNode::getString)
						.toArray(String[]::new));
		getSyntaxModule().loadData(configYml.node("syntax", "enabled").getBoolean(), configYml.node("syntax", "warn", "max").getInt(),
